...
Database schema migrations are automaticaly generated by comparing previous database schema versions and the current KeystoneJS lists. Incremental schema changes might be **immediately applied**, asked **one by one** or presented as **SQL statements**, so developers can review and have full control of their database schemas. 
