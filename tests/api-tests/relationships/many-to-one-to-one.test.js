...
const createInitialData = async keystone => {
  ...
  const custodians = await createItems({
    ...
    items: data.createLocations.map(({ id }) => ({
      data: { name: `Cu_of_${id}`, locations: { connect: [{ id }] } },
    })),
  });
    ...
    createLists: keystone => {
      ...
      keystone.createList('Location', {
        fields: {
          ...
          custodians: { type: Relationship, ref: 'Cu.locations', many: true },
        },
      });
      keystone.createList('Cu', {
        fields: {
          name: { type: Text },
      ...
      describe('Read', () => {
        ...
        test(
          ...
          runner(setupKeystone, async ({ keystone }) => {
            ...
            const { data, errors } = await keystone.executeGraphQL({
              query: `{
                  allCus(where: { locations_some: { company: { owners_some: { name: "${name1}" } } } }) { id locations { company { owners { name } } } }
                }`,
            });
            expect(errors).toBe(undefined);
            expect(data.allCus.length).toEqual(2);
          })
        );
        test(
          'Where C',
        ...
        test(
          ...
          runner(setupKeystone, async ({ keystone }) => {
            ...
            const { data, errors } = await keystone.executeGraphQL({
              query: `{
                  allCus(where: { locations_some: { company: { owners_some: { companies_some: { location: { custodians_some: { name: "${name1}" } } } } } } }) { id locations { company { owners { name } } } }
                }`,
            });
            expect(errors).toBe(undefined);
            expect(data.allCus.length).toEqual(2);
