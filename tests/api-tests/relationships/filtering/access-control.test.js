...
function setupKeystone(adapterName) {
  return setupServer({
    ...
    createLists: keystone => {
      keystone.createList('UToPLimitedRead', {
        fields: {
          username: { type: Text },
          posts: { type: Relationship, ref: 'PLimitedRead', many: true },
        },
      });

      keystone.createList('PLimitedRead', {
        fields: {
          name: { type: Text },
          content: { type: Text },
        },
      ...
      test(
        ...
        runner(setupKeystone, async ({ keystone }) => {
          ...
          const { data, errors } = await keystone.executeGraphQL({
            query: `
              query {
                UToPLimitedRead(where: { id: "${user.id}" }) {
                  id
                  username
                  posts {
                    id
