...
class MigrationExecution {
    ...
    async _updateAssociationCardinalitiesFlipForeignKeys(migration, referencedAssociationsState) {
        ...
        for(let i=0; i < rows.length; ++i) {
            ...
            await this._knex(migration.target.options.left.list)
                .where({ id: r[migration.before.options.left.field] })
                .update({ [migration.target.options.left.field]: r['id'] });
        }
    ...
    async _updateAssociationCardinalitiesFromSingle_N1_to_Referenced_11(migration, referencedAssociationsState) {
        ...
        await this._updateAssociationCardinalitiesFlipForeignKeys(migration, referencedAssociationsState);
    }
