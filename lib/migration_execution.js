    ...
    async _renameAssociation(migration, referencedAssociationsState) {
        ...
        if(!migration.target.referenced) {
            ...
            if(migration.cardinality === "N:N") {
                ...
                const beforePivotTableName = `${migration.name}_${migration.before.name}_many`;
                const pivotTableName = `${migration.name}_${migration.field.name}_many`;                                
...
*/
            ...
            if(migration.cardinality === "1:N") {
                ...
                // This is tricky since either way 
                // on database schema
                    
                await this._knex.schema.table(migration.target.list, (t) => {
                    //t.renameColumn(migration.before.options.refFieldPath, migration.field.options.refFieldPath);                    
                });                               
    ...
    async _removeAssociation(migration, referencedAssociationsState) {
        ...
        if(!migration.target.referenced) {
            ...
            if(migration.cardinality === "N:1") {
                ...
                await this._knex.schema.table(migration.name, (t) => {
                    // It seems foreign key is droped as well
                    t.dropColumn(migration.field);
                });                
            }
            ...
            if(migration.cardinality === "1:N") {                
                await this._knex.schema.table(migration.target.list, (t) => {
                    t.dropColumn(referencedMigration.migration.field);
                });
