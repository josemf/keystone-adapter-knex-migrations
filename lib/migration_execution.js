    ...
    async apply(migrations, schema, cmd = "create", cachedSchemaId = undefined) {
        ...
        if(orderedMigrations.length > 0) {
            ...
            if(cmd === "initial") {
                ...
                this._log.info("Those migrations represent your initial database schema:");
            } else {
            ...
            const confirmed = cmd === "initial" || this._options.mode !== 'migrate' || await this._log.confirm("You want to proceed?");

            if(confirmed) {                
                
                for(const migration of orderedMigrations) {

                    if(cmd === "initial") continue;
