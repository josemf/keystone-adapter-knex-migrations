...
class MigrationExecution {
        ...
        return migrations
            .filter(m => m.object === "association")
              .reduce((a, m) => {
                  ...
                  const target = m.before ? m.target.options.target : m.target;
                ...
                if(target.referenced) {
                    ...
                    const targetKey = `${target.list}__${target.referenced}`;
    ...
    async _updateAssociationCardinalitiesFromSingle_NN_to_Referenced_11(migration, referencedAssociationsState) {
        await this._createAssociationRelationship(migration.target.options.left.list, migration.target.options.left.field, migration.target.options.right.list);
    ...
    async _updateAssociationCardinalitiesFromReferenced_11_to_Single_NN(migration, referencedAssociationsState) {
    ...
    // Here
    async _updateAssociationCardinalitiesFromSingle_NN_to_Referenced_1N(migration, referencedAssociationsState) {
    ...
    async _updateAssociationCardinalitiesFromReferenced_1N_to_Single_NN(migration, referencedAssociationsState) {
    ...
    async _updateAssociationCardinalitiesFromSingle_NN_to_Referenced_N1(migration, referencedAssociationsState) {
    ...
    async _updateAssociationCardinalitiesFromReferenced_N1_to_Single_NN(migration, referencedAssociationsState) {
    ...
    async _updateAssociationCardinalitiesFlipForeignKeys(migration, targetLeft, targetRight, beforeLeft, beforeRight, referencedAssociationsState) {
    ...
    async _updateAssociationCardinalitiesFromSingle_N1_to_Referenced_11(migration, referencedAssociationsState) {
        ...
        await this._updateAssociationCardinalitiesFlipForeignKeys(migration, migration.target.options.left, migration.target.options.right, migration.before.options.left, migration.before.options.right, referencedAssociationsState);
    }

    async _updateAssociationCardinalitiesFromReferenced_11_to_Single_N1(migration, referencedAssociationsState) {
        // We flip the foreign keys and try to migrate data around

        await this._updateAssociationCardinalitiesFlipForeignKeys(migration, migration.target.options.left, migration.target.options.right, migration.before.options.left, migration.before.options.right, referencedAssociationsState);
    }

    async _updateAssociationCardinalitiesFromSingle_N1_to_Referenced_N1(migration, referencedAssociationsState) {
        // We flip the foreign keys and try to migrate data around
        
        await this._updateAssociationCardinalitiesFlipForeignKeys(migration, migration.target.options.left, migration.target.options.right, migration.before.options.left, migration.before.options.right, referencedAssociationsState);
    }
    
    async _updateAssociationCardinalitiesFromReferenced_N1_to_Single_N1(migration, referencedAssociationsState) {
        // We flip the foreign keys and try to migrate data around
    ...
    async _updateAssociationCardinalitiesFromSingle_N1_to_Referenced_1N(migration, referencedAssociationsState) {
        ...
        await this._updateAssociationCardinalitiesFlipForeignKeys(migration, migration.target.options.right, migration.target.options.left, migration.before.options.left, migration.before.options.right, referencedAssociationsState);
    }

    async _updateAssociationCardinalitiesFromReferenced_1N_to_Single_N1(migration, referencedAssociationsState) {
        // We flip the foreign keys and try to migrate data around
        
        await this._updateAssociationCardinalitiesFlipForeignKeys(migration, migration.target.options.left, migration.target.options.right, migration.before.options.right, migration.before.options.left, referencedAssociationsState);
    }

    async _updateAssociationCardinalitiesFromSingle_N1_to_Referenced_NN(migration, referencedAssociationsState) {
    ...
    async _updateAssociationCardinalitiesFromReferenced_NN_to_Single_N1(migration, referencedAssociationsState) {
    ...
    async _updateAssociationCardinalitiesFromSingle_NN_to_Referenced_NN(migration, referencedAssociationsState) {
    ...
    async _updateAssociationCardinalitiesFromReferenced_NN_to_Single_NN(migration, referencedAssociationsState) {
        ...
        if(Object.keys(referencedAssociationsState).length > 0) {

            // In case both side turn single N:N
    ...
    async _updateAssociationCardinalitiesFromReferenced_11_to_Referenced_1N(migration, referencedAssociationsState) {
        // We just need to flip the foreign key and migrate that data
        
        await this._updateAssociationCardinalitiesFlipForeignKeys(migration, migration.target.options.right, migration.target.options.left, migration.before.options.left, migration.before.options.right, referencedAssociationsState);
    }

    async _updateAssociationCardinalitiesFromReferenced_1N_to_Referenced_11(migration, referencedAssociationsState) {
        // We just need to flip the foreign key and migrate that data
        
        await this._updateAssociationCardinalitiesFlipForeignKeys(migration, migration.target.options.left, migration.target.options.right, migration.before.options.right, migration.before.options.left, referencedAssociationsState);
    }

    async _updateAssociationCardinalitiesFromReferenced_11_to_Referenced_N1(migration, referencedAssociationsState) {
        // NOOP the keys here wont flip
    }

    async _updateAssociationCardinalitiesFromReferenced_N1_to_Referenced_11(migration, referencedAssociationsState) {
        // NOOP
    }
    
    async _updateAssociationCardinalitiesFromReferenced_11_to_Referenced_NN(migration, referencedAssociationsState) {
    ...
    async _updateAssociationCardinalitiesFromReferenced_NN_to_Referenced_11(migration, referencedAssociationsState) {
    ...
    async _updateAssociationCardinalitiesFromReferenced_1N_to_Referenced_N1(migration, referencedAssociationsState) {
        // Flip the foreign keys        
        await this._updateAssociationCardinalitiesFlipForeignKeys(migration, migration.target.options.left, migration.target.options.right, migration.before.options.right, migration.before.options.left, referencedAssociationsState);        
    }

    async _updateAssociationCardinalitiesFromReferenced_N1_to_Referenced_1N(migration, referencedAssociationsState) {
        // Flip the foreign keys
        await this._updateAssociationCardinalitiesFlipForeignKeys(migration, migration.target.options.right, migration.target.options.left, migration.before.options.left, migration.before.options.right, referencedAssociationsState);              
    }

    async _updateAssociationCardinalitiesFromReferenced_1N_to_Referenced_NN(migration, referencedAssociationsState) {
        // This will create a pivot table copy data and remove the foreign key column
    ...
    async _updateAssociationCardinalitiesFromReferenced_NN_to_Referenced_1N(migration, referencedAssociationsState) {
        await this._createAssociationRelationship(migration.target.options.right.list, migration.target.options.right.field, migration.target.options.left.list);        
    ...
    async _updateAssociationCardinalitiesFromReferenced_N1_to_Referenced_NN(migration, referencedAssociationsState) {
        await this._createAssociationRelationship(migration.target.options.left.list, migration.target.options.left.field, migration.target.options.right.list, "id", migration.target.options.right.field, true);
    ...
    async _updateAssociationCardinalitiesFromReferenced_NN_to_Referenced_N1(migration, referencedAssociationsState) {
        await this._createAssociationRelationship(migration.target.options.left.list, migration.target.options.left.field, migration.target.options.right.list);        
    ...
    async _updateAssociationCardinalities(migration, referencedAssociationsState) {
        ...
        */
        ...
        if(referencedAssociationsState) {
            
            const referencedMigration    = referencedAssociationsState[`${migration.target.options.target.list}__${migration.target.options.target.referenced}`];
            const ownReferencedMigration = referencedAssociationsState[`${migration.name}__${migration.field}`];
            
            if(referencedMigration && referencedMigration.takenCare) {                                
                return;
            }
        }
        ...
        if(!migration.before.options.target.referenced && migration.before.options.cardinality === "N:1") {            
            ...
            if(migration.target.options.target.referenced && migration.target.options.cardinality === "1:1") {
                await this._updateAssociationCardinalitiesFromSingle_N1_to_Referenced_11(migration, referencedAssociationsState);
            }                        
            ...
            if(migration.target.options.target.referenced && migration.target.options.cardinality === "N:1") {
                await this._updateAssociationCardinalitiesFromSingle_N1_to_Referenced_N1(migration, referencedAssociationsState);
            }
            
            if(migration.target.options.target.referenced && migration.target.options.cardinality === "N:N") {
                await this._updateAssociationCardinalitiesFromSingle_N1_to_Referenced_NN(migration, referencedAssociationsState);
            }            
        }
        ...
        if(!migration.before.options.target.referenced && migration.before.options.cardinality === "N:N") {
            ...
            if(migration.target.options.target.referenced && migration.target.options.cardinality === "1:1") {
                await this._updateAssociationCardinalitiesFromSingle_NN_to_Referenced_11(migration, referencedAssociationsState);
            }

            if(migration.target.options.target.referenced && migration.target.options.cardinality === "1:N") {
                await this._updateAssociationCardinalitiesFromSingle_NN_to_Referenced_1N(migration, referencedAssociationsState);
            }

            if(migration.target.options.target.referenced && migration.target.options.cardinality === "N:1") {
                await this._updateAssociationCardinalitiesFromSingle_NN_to_Referenced_N1(migration, referencedAssociationsState);
            }

            if(migration.target.options.target.referenced && migration.target.options.cardinality === "N:N") {
                await this._updateAssociationCardinalitiesFromSingle_NN_to_Referenced_NN(migration, referencedAssociationsState);
            }                                                                        
        }
        ...
        if(migration.before.options.target.referenced && migration.before.options.cardinality === "1:1") {            
            if(!migration.target.options.target.referenced && migration.target.options.cardinality === "N:1") {                
                await this._updateAssociationCardinalitiesFromReferenced_11_to_Single_N1(migration, referencedAssociationsState);
            }

            if(!migration.target.options.target.referenced && migration.target.options.cardinality === "N:N") {                
                await this._updateAssociationCardinalitiesFromReferenced_11_to_Single_NN(migration, referencedAssociationsState);
            }

            if(migration.target.options.target.referenced && migration.target.options.cardinality === "1:N") {
                await this._updateAssociationCardinalitiesFromReferenced_11_to_Referenced_1N(migration, referencedAssociationsState);
            }

            if(migration.target.options.target.referenced && migration.target.options.cardinality === "N:1") {
                await this._updateAssociationCardinalitiesFromReferenced_11_to_Referenced_N1(migration, referencedAssociationsState);
            }

            if(migration.target.options.target.referenced && migration.target.options.cardinality === "N:N") {                
                await this._updateAssociationCardinalitiesFromReferenced_11_to_Referenced_NN(migration, referencedAssociationsState);
            }                                    
        }        
        ...
        if(migration.before.options.target.referenced && migration.before.options.cardinality === "1:N") {            
            ...
            if(!migration.target.options.target.referenced && migration.target.options.cardinality === "N:N") {                
                await this._updateAssociationCardinalitiesFromReferenced_1N_to_Single_NN(migration, referencedAssociationsState);
            }

            if(migration.target.options.target.referenced && migration.target.options.cardinality === "1:1") {
                await this._updateAssociationCardinalitiesFromReferenced_1N_to_Referenced_11(migration, referencedAssociationsState);
            }

            if(migration.target.options.target.referenced && migration.target.options.cardinality === "N:1") {
                await this._updateAssociationCardinalitiesFromReferenced_1N_to_Referenced_N1(migration, referencedAssociationsState);
            }

            if(migration.target.options.target.referenced && migration.target.options.cardinality === "N:N") {
                await this._updateAssociationCardinalitiesFromReferenced_1N_to_Referenced_NN(migration, referencedAssociationsState);
            }                                                
        }
        ...
        if(migration.before.options.target.referenced && migration.before.options.cardinality === "N:1") {            
            if(!migration.target.options.target.referenced && migration.target.options.cardinality === "N:1") {                
                await this._updateAssociationCardinalitiesFromReferenced_N1_to_Single_N1(migration, referencedAssociationsState);
            }

            if(!migration.target.options.target.referenced && migration.target.options.cardinality === "N:N") {                
                await this._updateAssociationCardinalitiesFromReferenced_N1_to_Single_NN(migration, referencedAssociationsState);
            }

            if(migration.target.options.target.referenced && migration.target.options.cardinality === "1:1") {
                await this._updateAssociationCardinalitiesFromReferenced_N1_to_Referenced_11(migration, referencedAssociationsState);
            }

            if(migration.target.options.target.referenced && migration.target.options.cardinality === "1:N") {
                await this._updateAssociationCardinalitiesFromReferenced_N1_to_Referenced_1N(migration, referencedAssociationsState);
            }

            if(migration.target.options.target.referenced && migration.target.options.cardinality === "N:N") {                
                await this._updateAssociationCardinalitiesFromReferenced_N1_to_Referenced_NN(migration, referencedAssociationsState);
            }                                                
        }
        ...
        if(migration.before.options.target.referenced && migration.before.options.cardinality === "N:N") {            
            if(!migration.target.options.target.referenced && migration.target.options.cardinality === "N:1") {                
                await this._updateAssociationCardinalitiesFromReferenced_NN_to_Single_N1(migration, referencedAssociationsState);
            }

            if(!migration.target.options.target.referenced && migration.target.options.cardinality === "N:N") {
                await this._updateAssociationCardinalitiesFromReferenced_NN_to_Single_NN(migration, referencedAssociationsState);
            }

            if(migration.target.options.target.referenced && migration.target.options.cardinality === "1:1") {
                await this._updateAssociationCardinalitiesFromReferenced_NN_to_Referenced_11(migration, referencedAssociationsState);
            }

            if(migration.target.options.target.referenced && migration.target.options.cardinality === "1:N") {
                await this._updateAssociationCardinalitiesFromReferenced_NN_to_Referenced_1N(migration, referencedAssociationsState);
            }

            if(migration.target.options.target.referenced && migration.target.options.cardinality === "N:1") {
                await this._updateAssociationCardinalitiesFromReferenced_NN_to_Referenced_N1(migration, referencedAssociationsState);
            }            
        }                        
    }
