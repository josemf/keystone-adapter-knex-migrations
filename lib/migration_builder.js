        ...
        this._schemaCurrent.forEach((listSchema, listName) => {            
            ...
            if(!cachedSchema) {               
                ...
                renameField.forEach(fieldSchema => {
                    ...
                    if(fieldSchema.target.type === "Relationship") {
                        this._renameAssociation(fieldSchema.source.name, listSchema.list, { }, fieldSchema.target, fieldSchema.source);
                    } else {
...
`);
    ...
    _buildFields(fieldAdapters) {
        ...
        return fieldAdapters
            .map(fieldAdapter => {
/*
                if(fieldAdapter.path === "category") {
                    console.log(fieldAdapter);   
                }               
                */
    ...
    _renameAssociation(name, tableName, options, field, before) {
        this._migration("association", "rename", tableName, { options, field, before, cardinality: field.options.cardinality, target: {
            list: field.options.refListKey,
            referenced: field.options.refFieldPath            
        }});        
    }
