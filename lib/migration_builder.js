...
class MigrationBuilder {
    ...
    constructor(listAdapters, knex, options = { cacheSchemaTableName: "InternalSchema", ignoreCacheSchema: false, silent: false }) {
        ...
        this._log = new CliLog(options.spinner, options.silent);
    }
...
`);
            ...
            this._log.error(`Also make sure you have an existing ./compiled folder`);
    ...
    _overrideTableSchemaOptions(fieldConfigurations) {
        return fieldConfigurations.map(fieldConfig => {
            ...
            if("text" === fieldConfig.method && fieldConfig.chainables.some(c => 'unique' === c.name)) {
