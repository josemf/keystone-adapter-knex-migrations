...
class CliLog {
    ...
    _buildOutputAssociation(association) {
        ...
        if('create' === association.op || 'remove' === association.op) {
            ...
            // TODO: Fix this for relation changes
            
            if(!association.before.reference) {
//
            } else {
            
                output = `RELATION ${chalk.dim(association.before.cardinality)} BETWEEN ${chalk.underline.bold(association.name)} ${chalk.underline(association.before.name)} AND ${chalk.underline.bold(association.before.reference.list)}${association.before.reference.field ? ' ' + chalk.underline(association.before.reference.field) : ''} \u2192 ${chalk.dim(association.target.cardinality)} BETWEEN ${chalk.underline.bold(association.name)} ${chalk.underline(association.target.name)} AND ${chalk.underline.bold(association.target.reference.list)}${association.target.reference.field ? ' ' + chalk.underline(association.target.reference.field) : ''}`;
