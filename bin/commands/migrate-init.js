...
const initMigrations = async (args, entryFile, spinner) => {
    ...
    if (!keystone.adapter.migrateInit) {
    ...
    try {
        await keystone.adapter.migrateInit(spinner, { mode: args['--mode'] || 'migrate', sqlPath: args['--sqlPath'] ? path.resolve(args['--sqlPath']) : undefined });
    } catch (e) {
...
module.exports = {
  ...
  `,
    exec: async (args, { exeName, _cwd = process.cwd() } = {}, spinner) => {
        ...
        spinner.start(' ');        
        return initMigrations(args, entryFile, spinner);
    },
};
